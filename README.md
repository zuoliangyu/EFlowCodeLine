...
| 平台 | 文件名 | 说明 |
|------|--------|------|
| macOS x64 | `eflowcodeline-macos-x64.tar.gz` | Intel Mac |
| macOS ARM64 | `eflowcodeline-macos-arm64.tar.gz` | Apple Silicon (M1/M2/M3/M4) |
| Linux x64 | `eflowcodeline-linux-x64.tar.gz` | 标准 x86_64 Linux |
| Linux x64 静态 | `eflowcodeline-linux-x64-static.tar.gz` | musl 静态链接，兼容低版本 glibc |
| Linux ARM64 | `eflowcodeline-linux-arm64.tar.gz` | 树莓派 4/5、Jetson、RK3588 等 |
| Linux ARM64 静态 | `eflowcodeline-linux-arm64-static.tar.gz` | ARM64 musl 静态链接 |
...
eflowcodeline --config      # 打开交互式 TUI 配置面板（查看用）
```
